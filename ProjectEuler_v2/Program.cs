    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            switch (problemId)
            {
                ...
                case 011:
                    ...
                    break;
                case 012:
                    problem_012(); // Highly divisible triangular number.
                    break;
                case 013:
                    problem_013(); // Large sum.
                    break;
                case 014:
        ...
        static void problem_012()
        {
            ...
            */
            ...
            bool found = false; // Did we find the number? 
            int result = 0; // Result. 
            int triNum = 0; // A triangle number. 
            int triNum2; // A variable to operate with a triangle number. 
            int divisorsAmt = 500; // Limit. 
            List<int> primes = new List<int>(); // Container of primes. 
            primes.Add(2); // Added the 2 to the list of primes. 
            primes.Add(3); // Added the 3 to the list of primes. 

            // Go through triangle numbers. 
            for (int i = 1; !found; i++)
            {
                triNum += i; // Next triangle number. 
                triNum2 = triNum; // Assign the number to the spare variable. 
                int divisorsAmtFound = 1; // Amount of divisors of a number. 

                // Container of amounts of prime divisors of the triangle number. 
                List<int> primeDivisorsAmt = new List<int>();
                primeDivisorsAmt.Add(0);

                // Go through prime numbers. 
                for (int j = 0; j < primes.Count; j++)
                {
                    // While the triangle number is divisible by the prime, divide it, update the list of divisors and reassign the triangle number. 
                    while (triNum2 % primes[j] == 0)
                    {
                        triNum2 = triNum2 / primes[j];
                        primeDivisorsAmt[primeDivisorsAmt.Count - 1]++;
                    }

                    // If the number is 1 end, else continue. 
                    if (triNum2 == 1)
                    {
                        break;
                    }
                    ...
                    {
                        // Add a counter for the new prime divisor. 
                        if (primeDivisorsAmt.Last() != 0)
                        {
                            primeDivisorsAmt.Add(0);
                        }

                        // If there no enough primes add next 10. 
                        if (j == primes.Count - 1)
                        {
                            int lastPrime = primes.Last();
                            int newPrimes = 0; // Amount of new primes. 

                            // Go through odd numbers, check for primerity, add to list if prime. 
                            for (int checkNumber = lastPrime + 2; newPrimes <= 10; checkNumber += 2)
                            {
                                bool isPrime = true; // The number is prime until the opposite has been proven. 
                                double sqrtCheckNumber = Math.Sqrt(checkNumber);

                                int primesCount = primes.Count; // Qty of already primes found. 

                                // Go through found primes and try to divide the checkNumver to it.
                                for (int k = 0; k < primesCount; k++)
                                {
                                    // No sense to take the primes greater than sqrt of the number.
                                    if (primes[k] > sqrtCheckNumber) break;

                                    // If checkNumber % prime == 0 then break. 
                                    if (checkNumber % primes[k] == 0)
                                    {
                                        isPrime = false;
                                        break;
                                    }
                ...
                // Calculate the amount of divisors. 
                for (int l = 0; l < primeDivisorsAmt.Count; l++)
                {
                    divisorsAmtFound *= (primeDivisorsAmt[l] + 1);
                }

                // Check if found. 
                if (divisorsAmtFound > divisorsAmt)
                {
                    found = true;
                    result = triNum;
                }
        ...
        static void problem_013()
        {
            ...
            String[] separator = { " " };
            int count = 100;
            String[] numbers = input.Split(separator, count, StringSplitOptions.RemoveEmptyEntries);
        ...
        // Longest Collatz sequence. Helper method. 
        static int Sequence(double n, int chain)
        {
            // Console.Write(n + " - ");
            if (n == 1)
            {
